...
- New from P6-2: whether the report's `evidence` field should also expose fragment source content (currently only denial/unresolved/omission decisions and counts) is an unresolved product decision, returned to the orchestrator rather than decided by the implementer — see P6-2's own evidence bullet above.
- New from P6-2: a fresh (non-resumed) run's `runId` never reaches `AuditResult`/the report — only a `--resume`d run's `resume.runId` does. A report cannot be looked up again by id without either threading `runId` through `AuditResult` unconditionally or adding the still-undesigned `reports` table.

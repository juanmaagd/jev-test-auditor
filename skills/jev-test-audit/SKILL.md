...
2. Explain needs-change count/share with its denominator (misleading/weak only), plus the separate needs-review share over the same denominator — never present needs-review as part of "needs a change"; run `dimensions`/`folders` for the worst ones, explained via `references/dimensions.md`.
3. Run `batches --by file` (or `--by folder [--max-tests N]`). Present the plan, ask which batches to fix, then stop and wait.
4. Per approved file: run `file <path>` (compact by default — only the weak/misleading tests and their offending dimensions), inline it into `assets/fixer-brief.md`'s findings placeholder and paste the matching `references/dimensions.md` section(s) into its dimension-guidance placeholder, dispatch one fix subagent per file. Respect the concurrency cap.
...
Return: the needs-change summary (count/share/denominator, worst folders/dimensions) and the needs-review share (same denominator, kept separate), the proposed batches and which were approved, each subagent's file/status/summary, the test command's result, and any `diff` comparison run. Never print full report JSON or an unapproved edit.

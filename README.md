...
- **Not yet built**: `--resume` and adaptive scheduling — see "Delivery phases" below.
...
| 5. Persistence, caching, and resilience | SQLite run store and cache, `--fresh`/resume, adaptive scheduling, and provider-throttling resilience. | **In progress** — SQLite run store (task P5-1) and content-addressed caching/`--fresh` (task P5-2) shipped, see "Audit store" and "Content-addressed caching" above; `--resume` and adaptive scheduling not yet built |
...
- Phases 2 and 3 emit structural test understanding and local evidence bundles only, with no network access. Phase 4 adds real Jev evaluation and classification, opt-in only via `--evaluate`. Phase 5 adds local SQLite persistence and content-addressed caching of evaluation results, also behind `--evaluate` (see "Audit store" and "Content-addressed caching" above); `--resume`, adaptive scheduling, cache-aware `--dry-run`, HTML reports, and benchmarks/calibration remain future work.

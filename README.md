...
| `audit --evaluate` | Opt-in only. Sends every evaluable test case's local evidence bundle to TypeSafe's Jev model for a real semantic judgment, replacing the normal summary with a terminal evaluation report (status counts, skipped-by-reason, failed count, total usage input tokens, and the responded model id), and persists the run locally to SQLite (see "Audit store" below). An evaluable test case whose content-addressed cache key already has a stored judgment is served from the local store instead of a new request (see "Content-addressed caching" below), reported as `Cached: N` rather than `Evaluated: N`. Requires `TYPESAFE_API_KEY`; its absence is a usage error (exit 1, no network attempted). Cannot be combined with `--dry-run` or `--inspect-payloads`. See "Jev evaluation" below. |
...
Before dispatching a real request, `audit --evaluate` computes a content-addressed cache key for each evaluable test case and looks it up in the audit store. A hit reuses the stored judgment at zero cost, recorded as its own `cached` work item; a miss dispatches a real request exactly as before, and the new judgment is recorded with that key so a later run can find it.

- **What the key covers.** The key is `sha256` over: the exact canonical Jev request bytes (`canonicalizeJevRequest`'s `{ state, model, questions }` — this already includes the pinned model id, every rubric question's wording, and the evidence bundle's selected fragments/denials/omissions), the normalized full test-file source (not only the fragments evidence selection happened to choose — an edit anywhere else in the file still invalidates), the rubric's own numeric version, and the classification policy's own numeric version. Changing any one of these five inputs forces a new request; the model id and evidence-fragment invalidation come for free from the canonical request bytes, since the model id is `request.model` and a changed fragment changes `state.fragments` directly.
...
- **Not yet built**: the dry-run estimator (`audit --dry-run`) does not yet consult the cache, so its billable-request count does not yet subtract cache hits — see "Delivery phases" below.
...
- **Named, visible failure modes — never a silent fresh start.**
  ...
  - A run id recorded against a different `--rootDir` than the one being audited now: usage error, exit 1, naming both roots.
...
- `audit --evaluate` (see below) sends this exact same request and reports exact usage from the provider's own response. `--dry-run` does not yet consult the audit store's cache (task P5-5), so its `initialCalls` counts every evaluable test case as billable even when a warm cache would actually serve some of them for free — see "Content-addressed caching" below.
...
| 5. Persistence, caching, and resilience | SQLite run store and cache, `--fresh`/resume, adaptive scheduling, and provider-throttling resilience. | **In progress** — SQLite run store (task P5-1), content-addressed caching/`--fresh` (task P5-2), adaptive scheduling with request/token budgets (task P5-3), and `--resume <runId>` (task P5-4) shipped, see "Audit store", "Content-addressed caching", "Adaptive scheduling and resilience", and "Resume" above; cache-aware `--dry-run` (task P5-5) not yet built |

...
- **Costs money and sends evidence to TypeSafe.** Every evaluable test case's local evidence bundle (test body plus its minimal helper/production-seam fragments and provenance — never the whole repository, never audited code executed) is sent as one Jev request, batched with all 14 rubric questions (`RUBRIC_V1`). Pricing is USD 0.042 per 1,000,000 input tokens (output tokens are unbilled); see `JEV_ESTIMATE_SNAPSHOT` in `src/domain/estimate.ts` and `audit --dry-run` for a no-network cost preview before spending anything for real. **The rubric's own questions dominate a request's bytes, not the evidence** — the first real run (11 requests, 2026-09-20) measured the rubric text at ~93% of the average request's bytes, which is why `--dry-run` measures the real request instead of estimating from evidence bytes alone (see below).
